...
export async function POST(request: Request) {
  ...
  logger.info("Margin segment analysis request received", {
  ...
  try {
    ...
    const segment_fields = body?.segment_fields ?? ["customer_segment", "product_category", "region"]
    const analysis_type = body?.analysis_type ?? "margin_by_segment"

    const sampleData = data.slice(0, 50)
    const dataString = JSON.stringify({ columns, data: sampleData }, null, 2)
    const prompt = `Analyze margin performance by customer segments:\n\nData:\n${dataString}\n\nSegment fields: ${JSON.stringify(segment_fields)}\nAnalysis type: ${analysis_type}\n\nIdentify which customer segments or product categories have the worst margin performance.\n\nProvide analysis in this JSON format:\n{\n  "segments_analyzed": number,\n  "worst_segment": "segment_name",\n  "best_segment": "segment_name", \n  "segment_performance": [\n    {\n      "segment": "Enterprise",\n      "avg_margin_pct": 15,\n      "revenue": 50000,\n      "transaction_count": 100,\n      "margin_rank": "poor"\n    }\n  ],\n  "insights": ["Enterprise customers have lowest margins at 15%", "Small business segment most profitable"],\n  "sql": "SELECT query to analyze segments",\n  "samples": [{"segment": "Enterprise", "avg_margin": 15, "revenue": 50000}]\n}`
    ...
    try {
      ...
      parsedAnalysis = {
        segments_analyzed: 3,
        worst_segment: "Enterprise",
        best_segment: "Small Business",
        segment_performance: [
          {
            segment: "Enterprise",
            avg_margin_pct: 15,
        ...
        insights: ["Enterprise customers have lowest margins at 15%", "Small business segment most profitable"],
        sql: "SELECT customer_segment, AVG((net_price - cost) / net_price * 100) as avg_margin_pct FROM pricing_data GROUP BY customer_segment ORDER BY avg_margin_pct ASC",
    ...
    logger.error("Segment analysis processing error", { request_id: requestId, ...details })
    return NextResponse.json(
      {
