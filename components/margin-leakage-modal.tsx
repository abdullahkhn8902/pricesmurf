...
export function MarginLeakageModal({ isOpen, onClose }: MarginLeakageModalProps) {
  ...
  const mockFiles = [
    { id: "pricing_data.csv", name: "pricing_data.csv", type: "CSV", readOnly: true },
    { id: "transactions_with_costs.xlsx", name: "transactions_with_costs.xlsx", type: "XLSX", readOnly: true },
  ]

  const isObjectId = (id?: string) => typeof id === "string" && /^[a-fA-F0-9]{24}$/.test(id)

  // Is real selection now requires file to be present in list AND be an ObjectId (prevents running on mock filenames)
  const isRealSelection = !!files.find((f) => String(f.id) === String(selectedFile)) && isObjectId(selectedFile)
  ...
  const handleRunAgent = async () => {
    ...
    try {
      ...
      // 2) validate (send inline preview to /api/validate-margin)
      const validateRes = await fetch("/api/validate-margin", {
        method: "POST",
      ...
      const createRunRes = await fetch("/api/margin-report", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      // MarginLoaderSequence (rendered) will pick the runId prop and start work
    } catch (err: any) {
  ...
  const handleLoadingComplete = async (finalResults: any, loaderRunId?: string) => {
    const useRunId = loaderRunId || serverRunId || `run_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`

    try {
      const serialized = JSON.stringify(finalResults)
      try {
        // Write canonical key expected by results page
        localStorage.setItem(`margin_results_${useRunId}`, serialized)
      } catch (e) {
        console.warn("Failed to write final results to localStorage", e)
      }

      // Try persist to server (idempotent upsert)
      await fetch("/api/margin-report/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        {modalState === "file-selection" && (
          <div className="space-y-6 py-4">
            <div className="space-y-2">
                ...
                <SelectContent>
                  ...
                  {!isLoadingFiles &&
                    (files.length ? files : mockFiles).map((file) => (
                      <SelectItem key={file.id} value={file.id} disabled={!!file.readOnly}>
                        <div className="flex items-center gap-2">
                          <FileSpreadsheet className="h-4 w-4" />
                          <span>{file.name}</span>
              ...
              {!isRealSelection && selectedFile && (
                <div className="text-sm text-muted-foreground mt-1">
                  Note: Selected file appears to be a demo file. Upload or select a real file to run analysis.
                </div>
              )}
            </div>
...
export default MarginLeakageModal
