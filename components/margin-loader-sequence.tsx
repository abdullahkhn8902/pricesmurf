// components/margin-loader-sequence.tsx
"use client"
...
export function MarginLoaderSequence({ fileId, runId, onComplete, onError }: MarginLoaderSequenceProps) {
  ...
  const runStep = async (stepIndex: number) => {
    ...
    try {
      ...
      const bodyPayload: any = {
        ...
        ...(step.id === "pricing" && { analyze_fields: ["list_price", "net_price", "discount_pct"], price_thresholds: { min_price: 0, max_discount: 100 } }),
        ...(step.id === "costs" && { cost_fields: ["cost", "cogs"], margin_thresholds: { min_margin_pct: 10, target_margin_pct: 30 } }),
        ...(step.id === "leakage" && { leakage_rules: ["net_price < cost", "margin_pct < 5", "discount_pct > 50"], priority_threshold: 1000 }),
        ...(step.id === "segments" && { segment_fields: ["customer_id", "customer_segment", "product_category"], analysis_type: "margin_by_segment" }),
        ...(step.id === "recommendations" && { recommendation_types: ["pricing_optimization", "cost_reduction", "segment_targeting"] }),
      }
  ...
  const finalizeAndReturn = async () => {
    ...
    const finalResults: any = {
      ...
      top_product_losses: (acc.recommendations?.top_product_losses) ?? (acc.leakage?.top_product_losses) ?? (acc.pricing?.top_product_losses) ?? [],
      ...
      samples: acc.samples ?? acc.pricing?.samples ?? acc.leakage?.samples ?? { below_cost: [], low_margin: [], extreme_discount: [] },
      remediation_suggestions: acc.recommendations?.remediation_suggestions ?? acc.recommendations?.recommendations ?? [],
      severity_summary: acc.segments?.severity_summary ?? acc.leakage?.severity_summary ?? { critical: 0, high: 0, medium: 0, low: 0 },
      sql_queries: {
  ...
  return (
    <div className="space-y-6 py-4">
      <div className="space-y-2">
        <div className="flex justify-between text-sm"><span>Margin Analysis Progress</span><span>{Math.round(progress)}%</span></div>
      ...
      <div className="space-y-3">
        {MARGIN_STEPS.map((step, index) => {
          ...
          return (
            <div key={step.id} className={cn("flex items-center gap-3 p-3 rounded-lg border transition-colors", isActive && "bg-muted/50 border-primary", result?.status === "success" && "bg-green-50 border-green-200", result?.status === "error" && "bg-red-50 border-red-200")}>
              ...
              <div className="flex-shrink-0 text-xs text-muted-foreground">{index + 1}/{MARGIN_STEPS.length}</div>
            </div>
          )
        })}
      </div>
      ...
      {hasError && (
        <Alert variant="destructive">
          ...
          <AlertDescription>{errorStep && `Failed at step: ${errorStep.label}. ${stepResults[errorStep.id]?.error}`}</AlertDescription>
        </Alert>
      )}
      ...
      {hasError && (
        <div className="flex gap-3">
          <Button onClick={handleRetry} disabled={isRetrying} size="sm">
            {isRetrying ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Retrying...</> : <><RotateCcw className="h-4 w-4 mr-2" />Retry</>}
          </Button>
          <Button variant="outline" size="sm" onClick={() => {
            const debugData = { fileId, runId, steps: stepResults, accumulated: accumRef.current, timestamp: new Date().toISOString() }
